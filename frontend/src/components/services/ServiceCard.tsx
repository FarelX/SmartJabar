...
export function ServiceCard({ service, onServiceClick, onEdit, onDelete }: ServiceCardProps) {
  ...
  return (
    <div className="h-full flex flex-col justify-between relative group overflow-hidden bg-white/90 backdrop-blur-md rounded-xl border border-slate-200/90 shadow-xs hover:shadow-md hover:border-primary-300 transition-all duration-200">
      {/* Admin controls */}
      {isAdmin && (
        <div className="absolute top-2.5 right-2.5 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity z-10">
          <Button
            ...
            size="icon"
            title="Edit Layanan"
            className="h-7 w-7 bg-white/95 border-slate-200 text-slate-600 hover:text-primary-600 hover:bg-primary-50 shadow-xs"
            onClick={(e) => {
              e.stopPropagation()
              onEdit?.(service)
            }}
          ...
          <Button
            ...
            size="icon"
            title="Hapus Layanan"
            className="h-7 w-7 bg-white/95 border-slate-200 text-slate-600 hover:text-red-600 hover:bg-red-50 hover:border-red-200 shadow-xs"
            onClick={(e) => {
              e.stopPropagation()
              onDelete?.(service)
            }}
      ...
      {/* Status badge */}
      {!service.is_active && (
        <Badge variant="outline" className="absolute top-2.5 left-2.5 text-[10px] border-red-200 text-red-600 bg-red-50 z-10 font-medium">
          Nonaktif
        </Badge>
      )}
      ...
      >
        ...
        <div className="w-full h-32 flex items-center justify-center p-4 bg-slate-50/70 group-hover:bg-blue-50/30 border-b border-slate-100 transition-all rounded-t-xl shrink-0">
          {service.icon_url ? (
            <img
        ...
        <div className="p-4 pt-3 flex-1 flex flex-col justify-between">
          ...
          <div className="flex items-center justify-between pt-2.5 border-t border-slate-100 mt-auto">
            <Badge variant="outline" className="text-[10px] border-slate-200 text-slate-600 bg-slate-50 font-normal shrink-0">
